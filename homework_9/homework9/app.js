...
function handle_new_skill_submit(){
    ...
    switch(skills_dropdown_btn_text_content.trim()){
        ...
        case "Runtime / Frameworks": {
            add_skill_to_skill_section_id("skills-section-runtimes-frameworks", skill_text);
            break;
        }
        case "Software": {
            add_skill_to_skill_section_id("skills-section-software", skill_text);
            break;
        }
        default: break;
    }
...
function get_table_html(columns, rows){
    ...
    const get_row_html = (row) => row.map(col => `<td>${col}</td>`).join('\n');
    const get_rows_html = (rows) => rows.map(row => `<tr>${get_row_html(row)}</tr>`).join('\n');
    const table_template = `
    <table class="table table-responsive table-striped">
        <thead>
            <tr>
                ${columns.map(get_column_html).join('\n')}
            </tr>
        </thead>
        <tbody>
            ${get_rows_html(rows)}
...
$('#experience-section').append(get_table_html(
    ["Position", "Institution", "Year(s)"], [
        ...
        ["Club Lead Software Engineer", "First Robotics", "2022-2025"]
    ]
));